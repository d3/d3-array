...
2D bins categorize many discrete samples into a smaller number of non-overlapping two dimensional intervals, sometimes called cells, laid out on a grid where x and y bins are non-overlapping intervals. They are often used to visualize the distribution of numerical data.
...
Same as [*bin*.value](#bin_value) for the X dimension. 

<a name="bin2d_value_y" href="#bin2d_value_y">#</a> <i>bin2d</i>.<b>valueY</b>([<i>valueY</i>]) [<>](https://github.com/d3/d3-array/blob/master/src/bin2d.js#L58 "Source")

Same as [*bin*.value](#bin_value) for the Y dimension. 
...
Same as [*bin*.domain](#bin_domain) for the X dimension. The default domain accessor is [extentX](#extent_x).

<a name="bin2d_domain_y" href="#bin2d_domain_y">#</a> <i>bin2d</i>.<b>domainY</b>([<i>domain</i>]) [<>](https://github.com/d3/d3-array/blob/master/src/bin2d.js#L62 "Source")

Same as [*bin*.domain](#bin_domain) for the Y dimension. The default domain accessor is [extentY](#extent_x).
...
Same as [*bin*.thresholds](#bin_thresholds) for the X dimension.
...
Same as [*bin*.thresholds](#bin_thresholds) for the Y dimension.

Any threshold values outside the [domainY](#bin2d_domain_y) are ignored. The first *bin*.y0 is always equal to the minimum domain value, and the last *bin*.y1 is always equal to the maximum domain value.

### 2D bin Thresholds

Identical to [Bin Thresholds](#bin-thresholds) above.