...
it("superset(values, other) returns true if values is a superset of others", () => {
  assert.equal(d3.superset([1, 2], [2]), true);
  assert.equal(d3.superset([2, 3], [3, 4]), false);
  assert.equal(d3.superset([1], []), true);
});

it("superset(values, other) allows values to be infinite", () => {
  assert.equal(d3.superset(odds(), [1, 3, 5]), true);
});

it("superset(values, other) allows other to be infinite", () => {
  assert.equal(d3.superset([1, 3, 5], repeat(1, 3, 2)), false);
});
