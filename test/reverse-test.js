...
it("reverse(values) returns a reversed copy", () => {
  ...
  assert.deepStrictEqual(d3.reverse(input), [4, 5, 2, 3, 1]);
...
it("reverse(values) accepts an iterable", () => {
  assert.deepStrictEqual(d3.reverse(new Set([1, 2, 3, 2, 1])), [3, 2, 1]);
  assert.deepStrictEqual(d3.reverse((function*() { yield* [1, 3, 2, 5, 4]; })()), [4, 5, 2, 3, 1]);
  assert.deepStrictEqual(d3.reverse(Uint8Array.of(1, 3, 2, 5, 4)), [4, 5, 2, 3, 1]);
});
...
it("reverse(values) does not skip sparse elements", () => {
  ...
  assert.deepStrictEqual(d3.reverse([, 1, 2,,]), [undefined, 2, 1, undefined]);
});
