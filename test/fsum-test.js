...
tape("fsum(array) returns an Adder", function(test) {
  test.equal(typeof arrays.fsum([1]).add, "function");
  test.equal(typeof arrays.fsum([1]).valueOf, "function");
  test.end();
});
...
tape("fsum(array) is an exact sum", function(test) {
  ...
  test.equal(+arrays.fsum().add(["20", "3"].map(box), unbox), 23);
  test.equal(+arrays.fsum([box("20")], unbox).add(3), 23);
  test.end();
});
